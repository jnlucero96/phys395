...
integer i
real dt
real x_init, Etot, half_period

! timestep resolving fastest timescale in the scan
dt = 1e-3

! populate initial position array
do i=1,50
    x_init = i/10.0
    Etot = V(x_init)
    half_period = integrate(x_init, 100.0, dt)

    write (*,*) Etot, 2.0*half_period
end do
...
function integrate(x, t, dt)
    real x, t, dt, integrate
    real u(2); integer i, n, counter
    real xdot_before
    ...
    do i = 1,n
        xdot_before = u(2)
        call gl10(u, dt)
        ! check for a switch in sign of the velocity since that means
        ! the velocity must have crossed zero
        if (xdot_before*u(2) < 0) then 
                exit
        else
                counter = counter + 1
        end if
    ...
    ! return how long it took for the switch to happen
    if (counter >= n) then
            ! return 0.0 if the velocity never flipped in the integration time
            integrate = 0.0
    else
            integrate = counter*dt
    end if
end function
