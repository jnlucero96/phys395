...
H = L; forall (i=1:n) H(i,i) = L(i,i) + V(x(i))

call lsolve(eig_vals_out, eig_vecs_out)

do ii=1,10
    write(file_out_index,*) -0.5*eig_vals_out(ii)
    do jj=1,n
...
subroutine initg()
    ...
    forall (i=1:n) theta(i) = pi*(n-i+0.5)/n; x = atanh(cos(theta))
end subroutine
...
subroutine evalb(n, pts, theta, Tn, Tnx, Tnxx)
        ...
        if (present(Tnx))  Tnx = n * sin(n*theta) * sin(theta)
        if (present(Tnxx)) Tnxx = -n*sin(theta)*(n*cos(n*theta)*sin(theta) + sin(n*theta)*cos(theta)) 
