...
real, dimension(n,1) :: f_of_x2, fprime_of_x2
...
do j=1,n
    ...
    fprime_of_x(j,1) = Dfunc_of_interest(x(j,1))
    f_of_x2(j,1) = func_of_interest(x(j,1))
    fprime_of_x2(j,1) = Dfunc_of_interest(x(j,1))
    do i=1,n
        ! populate matrices
        basis_matrix(i,j) = ChebyshevT(x(i,1),j)
        basis_prime_matrix(i,j) = DChebyshevT(x(i,1),j)
        basis_matrix2(i,j) = ChebyshevT(x(i,1),j)
        basis_prime_matrix2(i,j) = DChebyshevT(x(i,1),j)
    end do
end do
...
subroutine ChebGrid(array, n)
    ...
    integer :: i
    do i=1,n
        array(i,1) = cos((PI/n)*(i-0.5))
    end do
end subroutine ChebGrid
